...
#define  FLASH_DISK_BLOCK_NUM   128
#define  FLASH_DISK_BLOCK_SIZE  512


uint8_t disk_image[4][FLASH_DISK_BLOCK_SIZE] =
{
  {
      0xEB, 0x3C, 0x90,
      ...
      FLASH_DISK_BLOCK_NUM & 0xFF, FLASH_DISK_BLOCK_NUM >> 8,  // BPB_TotSec16
      0xF8,        // BPB_Media
...
//void __not_in_flash_func(flash_fat_initialize)(void) {
...
//bool __not_in_flash_func(flash_fat_read)(int block, uint8_t *buffer) {
bool flash_fat_read(int block, uint8_t *buffer) {
    const uint8_t *data = (uint8_t *)(XIP_BASE + FLASH_FAT_OFFSET + FAT_SECTOR_SIZE * block);
    memcpy(buffer, data, FAT_SECTOR_SIZE);
    return true;
}

//bool __not_in_flash_func(flash_fat_write)(int block, uint8_t *buffer) {
bool flash_fat_write(int block, uint8_t *buffer) {
    ...
    int flash_sector = floor((block * FAT_SECTOR_SIZE) / FLASH_SECTOR_SIZE);
    int flash_sector_fat_offset = (block * FAT_SECTOR_SIZE) % FLASH_SECTOR_SIZE;
    ...
    memcpy(data + flash_sector_fat_offset, buffer, FAT_SECTOR_SIZE);
